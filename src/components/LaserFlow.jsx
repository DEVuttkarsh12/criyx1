...
function hexToRGB(hex) {
  ...
  const numeric = Number.parseInt(color, 16) || 0xffffff;
...
export default function LaserFlow({
  ...
  useEffect(() => {
    ...
    const uniforms = {
      ...
      uFade: { value: disableAnimation || hasFadedRef.current ? 1 : 0 },
    };
    ...
    let previousTime = 0;
    let fade = disableAnimation || hasFadedRef.current ? 1 : 0;
    ...
    const setSizeNow = () => {
      ...
      const sizeChanged =
        Math.abs(width - last.width) > 0.5 || Math.abs(height - last.height) > 0.5;
    ...
    const scheduleResize = () => {
      ...
      resizeFrame = window.requestAnimationFrame(setSizeNow);
    };
    ...
    const adjustDprIfNeeded = (now) => {
      ...
      if (elapsed < 750 || disableAnimation) {
        return;
      }
      ...
      } else if (averageFps > upperThreshold && currentDprRef.current < baseDpr) {
    ...
    const animate = () => {
      ...
      if (!disableAnimation) {
        ...
        const instantFps = 1000 / Math.max(1, emaDtRef.current);
        ...
        if (!hasFadedRef.current) {
          ...
          fade = Math.min(1, fade + clampedDelta / fadeDuration);
  ...
  }, [decay, disableAnimation, dpr, falloffStart, flowSpeed, fogFallSpeed, fogIntensity, fogScale, horizontalBeamOffset, horizontalSizing, mouseSmoothTime, mouseTiltStrength, verticalBeamOffset, verticalSizing, wispDensity, wispIntensity, wispSpeed]);
